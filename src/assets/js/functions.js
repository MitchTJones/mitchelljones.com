const colors = {white:'#FFFFFF', grey:'#808080', black:'#000000', red:'#FF0000', green:'#00FF00', blue:'#0000FE', yellow:'#FFFF00', aqua:'#00FFFF', pink:'#FFC0CB', purple:'#800080', orange:'#FFA500', silver:'#C0C0C0'};
...
function custom(id, setup, onResize) {
	...
	let o = {cvs:c, ctx:c.getContext('2d')};
	parentScale(o.cvs);
	setup(o.cvs, o.ctx);
...
function gradients(hexes, f) {
	let mF = f*(hexes.length-1);
	let lI = Math.floor(mF);
	let uI = lI+1;
	let lC = hexes[lI];
	if (uI == hexes.length)
		uI = lI;
...
function lesser(than, input) {
	if (typeof input == 'object') {
		let output = new Vector(input.x, input.y);
		if (output.x < than)
			output.x = than;
		if (output.y < than)
			output.y = than;
...
function getWinSize() {
	let winSize = new Vector(window.innerWidth, window.innerHeight);
	return winSize;
}

function getCanvasSize() {
	let canSize = new Vector(canvas.width, canvas.height);
	return canSize;
}
